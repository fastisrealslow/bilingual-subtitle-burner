...
- `OUTPUT_LAYOUT=auto`：已验收的单段真人横向窗口优先重排为横版，不再按母片哈希抽取三分之一。原生画面、真实竖屏和未知窗口不强制转换；显式 `portrait/landscape` 保持优先。可选重排失败时，只能保留指纹和验收资产完全未变的原片。
- `LINYUAN_LANDSCAPE_STYLE=quiet` 为候选代码默认：深灰背景、784×584 真人窗口、44px 可见字幕字高及独立字幕带；`classic` 保留显式兼容。已绑定坐标的图表片仍走原图表版式。新版自动选择与默认样式是后续变化，不冒充冻结100条 `bfd29f6` 的实测结果。
...
- 时长默认使用 `LINYUAN_CONTENT_POLICY=reference_v1`：允许20秒以上的完整短观点、120～330秒完整问答和更长访谈，由原话自然边界决定。20秒只是技术下限，不是目标长度；没有完整观点仍拒绝。生产、选源、验收和投稿共用同一策略，新产物记录策略版本与类型。旧库存缺少策略标记时仍按120秒验收，不能借新规则自动转正。设置 `legacy120` 可恢复旧时长规则；固定100历史对照显式保持旧规则，新规则实验单列。
...
2026-09-24 的逐片复核覆盖冻结复验已返回的全部 17 份视频，记录见 `linyuan/simulations/benchmark-20260921/final100-editorial-review.json`；每条的标题方向、封面判断及下一步编辑方案见 `final100-editorial-actions.json`。这些是逐帧取样和全部字幕的编辑观察，不是全片逐秒听音通过，也不是点击率实验。

专项新版的观察绑定运行号、MP4 SHA256 和封面 SHA256：同一个视频可以有不同封面，不能只按视频指纹复用封面评语。独立 ASR 的 8 段原音及分歧显示在对应视频下，不会直接覆盖原字幕。

`LINYUAN_STATIC_CORNER_REPAIR=1` 只用于固定镜头的重裁实验，默认关闭。39 号实测未能在全部帧中同时避开墙中文字和保留完整人脸，因此没有采用 60.9 秒短剪；正式候选继续保留原来的 92.7 秒连续区间。此实验不增加出片统计，也不降低画面检查要求。
