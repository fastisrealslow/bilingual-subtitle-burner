...
def incident_entries(root):
    ...
    expected={
        'f2a561a9d6258ca6a4836abbbcbefb07af65029cf181e815b800a81b4669ab4c':[63,59],
        '31e694157feb6bd5f23bbfd1ca209ec35c37619bbe0de703547d391e9542535f':[75,61,91,95]}
    assert report['source_sha256'] in expected
    ...
    assert [len(x) for x in result]==expected[report['source_sha256']]
    return result
...
def main():
    ...
    if args.no_model:
        def forbidden(*a,**kw):raise AssertionError('Source caption replay must not require model inference')
        p.llm=forbidden
    ...
    for n,entries in enumerate(incident_entries(args.evidence),1):
        ...
        row=dict(part=n,screens=len(groups),seconds=round(time.monotonic()-started,2),passed=True,
                 model_disabled=args.no_model,character_conservation=True,word_and_layout_checked=True)
