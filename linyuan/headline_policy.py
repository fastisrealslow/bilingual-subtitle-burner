...
VERB = re.compile(r'判断|考虑|布局|核心|买|卖|投|持有|参与|研究|进入|到来|没来|涨|跌|赚|挣|亏|赔|值|回报|增长|增加|下降|风险|降价|涨价|机会|不|有|够|活|重要|便宜|贵|少|多|强|弱|完|老龄化|股息率')
...
def copy_length_ok(text, maximum=52):
    ...
    if not complete(value):
        return False
    # A short response can have an event as its subject and an emotion as
    # its predicate. Actual source34's “亏钱不高兴” was rejected solely
    # because it is neither a buy/sell command nor eight characters long.
    # This admits the form only; attribution and exact source support still
    # run later. Bare emotions or profit promises do not use this exception.
    reaction = r'(?:亏钱|赔钱|赚钱|挣钱)(?:我|也|就)?(?:不高兴|不开心|很开心|很难受|无所谓)'
    if re.fullmatch(reaction, compact(value)):
        return True
