...
def clean_interview_graphics(video,proof,cues,source_sha256,work):
    ...
    for a,b,_ in profile['spans']:
        replaced.update(n for n in range(max(0,int((a-start)*fps)),min(count,int((b-start)*fps+.999)))
                        if role(n) in {'guest','participant'})
    spans=[]
    ...
    if cards:
        ...
        for card in cards:cmd+=['-loop','1','-framerate',str(fps),'-i',card['path']]
    ...
    out={**proof,'mode':'verified_interview_context_v2','roles':roles,
        ...
        'source_timeline_preserved':True,'source_frames_preserved':not bool(cards),
