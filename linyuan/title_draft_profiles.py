...
title以“林园：”开头，正文4至52字；cover_title为4至18字的完整短句，不加姓名。短于8字须有明确对象和动作，不能仅列名词。标题和封面各自说明具体对象、同一个观点和必要限定，不截断；不补字填满版面。
...
拟稿按原声删繁：保留原句的主语、具体对象、动词、否定和语气，仅删口吃与没有信息的填充；对象是代词时，须从同一问答中已确认的嘉宾原话找出确实对应的名称补全，并把含名称的那一句也列入证据。中间有主持人的追问不妨碍引用已确认的后续嘉宾解释，但不能借用主持人的假设或改动说话人。
标题和封面都要让没看过视频的人知道在说什么。原因、标准、结果等泛词不能代替具体对象；不能只说没买、它不符合标准。没有证据补足对象时退回，不猜名称。
第一稿尽量贴近这一句原话；第二稿可将同一判断中的具体选择放在前面；第三稿可用完整问题引出同一回答。没有自然差异时允许相近，不为凑三种形式编理由。
...
title以“林园：”开头，正文4至52字；cover_title用4至18字的完整短句。短于8字须有明确对象和动作，不能仅列名词。一个判断说完整就可以，不凑第二句，不为填满版面加字。
