...
def main():
    ...
    if args.language == "auto" and full_srt.exists():
        ...
        if detected:
            ...
            # 中文源要多走一道规范化：Whisper 对台湾/香港片源经常输出繁体
            # （实测出“公佈”“調到了”），半全角标点也是随机混的，不处理就
            # 直接烧进画面。
            if detected == "zh":
                try:
                    sys.path.insert(0, str(ROOT / "scripts"))
                    from platform_rules import normalize_srt
                    normalize_srt(str(full_srt))
                except Exception as e:
                    print(f"[run] ⚠ 字幕规范化跳过：{e}", flush=True)
