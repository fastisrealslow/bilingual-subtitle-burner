...
def _once(messages, api_key, model, base_url, temperature):
    """对单一模型跑一次调用（退避重试在 ``sf_client`` 里）。

    可重试的失败耗尽后返回 None，交由上层换模型 —— 换个模型往往立刻就通了，
    这比继续死磕同一个拥堵端点有用。不可重试的错误（鉴权、余额、请求本身有
    问题）直接抛出：换几个模型都是一样的下场，只会白白拖时间。
    """
    ...
    try:
        ...
        print(f"[translate] {model} 持续失败：{e}", file=sys.stderr)
        return None
