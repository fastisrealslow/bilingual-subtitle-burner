...
<main><small>2026-09-23 续验 · 冻结9月21日批次 · 真实证据与待验部分分别标注</small><h1>先看能稳定做出多少，再看每条差在哪里</h1>
...
document.querySelectorAll('video').forEach(v=>v.onplay=()=>document.querySelectorAll('video').forEach(o=>{if(o!==v)o.pause()}));
    ...
    body = body.replace('<nav>', '<nav><a href="#all-outputs">当前全部实片（三列）</a><a href="#threeway">历史固定批次（三列）</a><a href="#overview">成功率总览</a><a href="#latest-results">实测与9B实验记录</a><a href="#title-sep22">标题实验</a><a href="#reference-speech">园园内容拆解</a><a href="#sources">素材来源</a><a href="#subtitles">字幕前后</a><a href="#packaging">标题封面</a><a href="#gap">园园差距</a>')
    overview=overview_sections(player)
    intro,separator,remaining=overview.partition('</section>')
    body = body.replace('<nav>', '<nav><a href="#final-batch">冻结候选完整复验</a>', 1)
