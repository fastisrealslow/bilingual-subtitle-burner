...
def sections(player):
    ...
    current=d['cohorts']['final100']
    inventory=read(ROOT/'output/final100-35902748400/results/media-inventory-audit.json',{})
    ...
    intro = '<section id="overview"><div class="eyebrow">整体验收 / 2026-09-24</div><h1>形式更接近了，稳定性还没达标。</h1><p class="lead">目标是每100条素材至少30条能稳定出片，并且内容值得发布。目前还不能交出这个结论。</p><p class="small">本地报告快照：'+esc(d['checked_at'])+'；主线数据 '+d['snapshot_main_sha'][:7]+'。页面不会自动更新；不同固定版本分别统计。</p>'+ \
        '<div class="metric-grid"><article><b>11/100</b><span>主线 · 同批素材技术出片</span></article><article><b>'+str(current['passed'])+'/100</b><span>冻结bfd29f6技术出片 · 已返回'+str(current['total']-current['missing'])+'/100</span></article><article><b>'+str(unique)+'组</b><span>冻结bfd29f6去重 · 非编辑合格数</span></article><article><b>30%</b><span>目标 · 尚未达到</span></article></div>'+ \
        '<p>顶部统计为冻结bfd29f6的完整100素材复验；后续17个成功素材用统一默认配置复跑17/17出片，仅证明这组已知素材的技术稳定性。逐片标题、封面、字幕与未解决问题见下方全部对照；专项改稿不混进固定批次成绩。</p><p><a href="#source-79">79：历史标题前后</a> · <a href="#source-17">17：字幕修复</a> · <a href="#source-8">8：发言归属</a> · <a href="#refs-latest">园园近期完整视频</a></p><details><summary>展开各轮完整统计与比较口径</summary>'+''.join(charts)+ \
    ...
    from linyuan_sep22_sections import sections as sep22_sections
    return intro+sep22_sections()+quality_iteration(player)+sources+subtitles+titles+gaps+fresh
