...
def build_threeway(reference_media, player):
    ...
    return '<section id="threeway"><h2>三列直接看：线上主线、优化版、园园</h2><p>左列是主线固定版本复跑，右列是园园原作，按主题或形式作参考，并非同素材实验。中列以06815b4固定批次为基础，8和17已更新到后续核验的实片，每条标明实际代码版本，原批次视频仍可展开播放。所有展示的成片已核对哈希、完整音视频解码、六帧及字幕；尚未逐秒听音验收。新8的主要回答未进标题、17的口吃与画面占比、34残字、4/68对象不明、79范围改写等问题照实保留。49保留本轮优化缺报告的空位。专项恢复只更新播放器，不回填17/100成绩。</p><p><button onclick="threewayFilter(\'all\')">全部</button> <button onclick="threewayFilter(\'both\')">两版都有成片</button> <button onclick="threewayFilter(\'new\')">自动出片新增（含质量问题）</button> <button onclick="threewayFilter(\'missing\')">优化未出片</button></p>'+''.join(cards)+'</section>'
