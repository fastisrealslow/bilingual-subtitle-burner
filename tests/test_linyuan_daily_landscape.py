...
def test_actual_landscape_productions_reserve_only_admission_not_finished_stock():
    ...
    inventory=dict(verified_landscape=0,daily_mix_usable=0)
    assert fc.landscape_admission_deficit(inventory,state,active)==0
    assert inventory==dict(verified_landscape=0,daily_mix_usable=0)
    # Failed/historical placeholders alone do not suppress new production.
    assert fc.landscape_admission_deficit(inventory,state,[])==2
    assert fc.landscape_admission_deficit(inventory,state,[{**active[0],'status':'completed'}])==2
    # A queued retry is one opportunity even if the same slug appears twice.
    assert fc.landscape_admission_deficit(inventory,state,[active[0],{**active[0],'id':99,'status':'queued'}])==1
    assert fc.landscape_admission_deficit(dict(verified_landscape=1),state,[active[0]])==0
    state['dispatched'][0]['weekly_full_week']='2026-W37'
    assert fc.landscape_admission_deficit(inventory,state,[active[0]])==2
