...
from title_impression_replay import impression_error


def test_rejects_both_actual_fields_of_the_frozen_source7_output():
    case = json.loads((ROOT / 'linyuan/simulations/benchmark-20260921/source78-frozen-title-corpus.json').read_text())[0]
    source = ''.join(c['text'] for c in case['cues'])
    assert impression_error(case['old_title'], '市场的估值判断', source)
    assert impression_error('市场的估值判断', case['old_cover'], source)
