...
def test_short_model_picks_must_select_from_real_long_contexts():
    ...
    verdicts['0']='accept_8'
    replies=['[{"start":0,"end":0,"score":8}]']*2+[json.dumps({'topics':[dict(start=0,topic='完整解释')]}),json.dumps({'verdicts':verdicts})]
    with tempfile.TemporaryDirectory() as tmp:
...
def test_review_cannot_reject_source_by_copying_nonexistent_example_words(tmp_path):
    ...
    assert result['issues']==[] and result['review_prompt_version']==7
